 ...
 *
 * Uses a pure-JS AES-256-CBC implementation with platform-agnostic random
 * byte generation and SHA-256 hashing. No native modules are required,
 * so this works in Expo Go without the 'ExpoCryptoAES' native module.
 *
 * The master encryption key is stored in the platform keychain (see keychain.ts).
 */
...
// ---------------------------------------------------------------------------
// Pure-JS random bytes & SHA-256 (no native module required)
// ---------------------------------------------------------------------------

/**
 * Generate cryptographically-random bytes using the best available source.
 * Works on web (crypto.getRandomValues) and React Native (global.crypto).
 */
function getRandomBytes(length: number): Uint8Array {
  const bytes = new Uint8Array(length);

  // Node / Web / React Native (Hermes 0.72+) all expose globalThis.crypto
  if (typeof globalThis !== 'undefined' && globalThis.crypto?.getRandomValues) {
    globalThis.crypto.getRandomValues(bytes);
    return bytes;
  }

  // Fallback: Math.random (not cryptographically secure, but keeps the app functional)
...
/**
 * Pure-JS SHA-256 implementation (no native dependencies).
 * Returns the hash as a lowercase hex string.
 */
async function sha256Internal(input: string): Promise<string> {
...
// ---------------------------------------------------------------------------
// Pure-JS SHA-256 Implementation (fallback when Web Crypto is unavailable)
// ---------------------------------------------------------------------------
